...
// Get get 请求
func Head(url string, options ...Option) (body []byte, err error) {
	return withoutBody(http.MethodHead, url, nil, options...)
}
...
func withoutBody(method, url string, form httpURL.Values, options ...Option) (body []byte, err error) {
	...
	if opt.trace != nil {
		opt.header[trace2.Header] = []string{opt.trace.ID()}
	}
	...
	if opt.dialog != nil {
		...
		opt.dialog.Request = &trace2.Request{
			TTL:        ttl.String(),
			Method:     method,
	...
	defer func() {
		...
		if nil != opt.logger {
			opt.logger.Info("api request:", zap.Any("info", opt.dialog))
		}
	}()
...
//记录请求信息
func logRequestInfo(opt *option, beginTime time.Time) {
	if nil == opt || nil == opt.dialog {
		return
	}
	if nil != opt.logger {
		opt.logger.Info("api request:", zap.Any("request info:", opt.dialog))
	}

	//doAfterRequest(opt,  beginTime, nil)
}

// doAfterRequest 记录请求日志信息到es 和 sqm
//func doAfterRequest(opt *option, beginTime time.Time, err error) {
//	midw := middleware2.GetInstance()
//	if nil == midw {
//		return
//	}
//	midwData := middle_struct.NewEmptyMiddleware()
//	midwData.TraceId = opt.trace.ID()
//	midwData.ReqBeginTime = beginTime
//	midwData.ReqEndTime = time.Now()
//	midwData.SetCategory(middle_struct.LogCategoryRequest)
//	if nil != err {
//		midwData.Level = zapcore.InfoLevel.String()
//	}
//	midwData.Req = opt.originRequest
//	midwData.DestUrl = opt.dialog.Request.DecodedURL
//	midwData.Data = opt.dialog
//	midw.DoAfter(midwData)
//}
